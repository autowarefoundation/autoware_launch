...
- `obstacle_segmentation/ground_segmentation_<backend>.launch.xml` — ground removal backends (`rule_based`, `rule_based_cuda`, `ml`); backend selection is file selection. `ground_segmentation_{additional_lidars,ransac}.launch.xml` fold extra lidars into the obstacle stream for kits that carry them.
